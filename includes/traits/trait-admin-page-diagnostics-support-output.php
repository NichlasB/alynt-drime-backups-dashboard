...
trait Alynt_Drime_Backups_Dashboard_Admin_Page_Diagnostics_Support_Output {
	...
	private function render_support_copy_output( array $support ) {
		...
		echo '</textarea><p class="adbd-actions"><button type="button" class="button button-primary adbd-copy-button" hidden data-copy-target="adbd-support-copy" data-busy-label="' . esc_attr__( 'Copying…', 'alynt-drime-backups-dashboard' ) . '" data-success-message="' . esc_attr__( 'Redacted support summary copied to the clipboard.', 'alynt-drime-backups-dashboard' ) . '" data-error-message="' . esc_attr__( 'The summary could not be copied automatically. Select it and copy it manually.', 'alynt-drime-backups-dashboard' ) . '">' . esc_html__( 'Copy Support Summary', 'alynt-drime-backups-dashboard' ) . '</button></p><p class="adbd-copy-status" role="status" aria-live="polite"></p></div></div>';
	}
}
