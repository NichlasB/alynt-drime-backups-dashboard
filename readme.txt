...
Version 0.1.46 includes pending-enrollment token generation, REST enrollment completion, credential-vault primitives, safe status-request preparation, first-poll activation, manual read-only status checks, scheduled read-only polling, dashboard-local scheduled polling pause/resume controls, dashboard-local archive/unarchive controls for revoked and expired pending records, bounded status-history retention, operator-focused admin views, redacted support diagnostics with aggregate dashboard record-state counts, optional structured diagnostics logging that is disabled by default, always-on redacted operator action history for dashboard-local actions with operator-friendly action labels, optional dashboard-side display of redacted per-source backup freshness evidence, redacted WPvivid source-activity hints, dashboard-side WPvivid freshness policy for weekly/biweekly schedules, schedule-aware WPvivid freshness-policy ingestion, a dashboard-owned per-site WPvivid external/optional monitoring policy for sites that intentionally handle WPvivid backups outside Alynt-uploaded evidence, clearer Sites-tab manual-check state copy, improved Sites-tab handling for action-button width and superseded revoked duplicates, stale-cache protection for read-only status polling, shorter manual-check button wording, aligned Sites-tab manual-check helper copy, copy-control busy-state polish, timestamp fallback hardening, malformed snapshot fail-closed behavior, safe default preservation of dashboard data during uninstall, V2.1 Request Backup Now for separately opted-in clients, V2.2 dashboard-side action-history reconciliation/audit hardening, Site Detail Remote Action History filters for action type and dashboard state, compact Remote Action History detail cells with longer support-safe details behind native disclosure, a patch fix that stores the signed action UUID as the dashboard action record public ID for newly dispatched actions, same-origin dashboard self-action safety for managed-host loopback/private DNS resolution, V2.3 preview-only schedule visibility for the Alynt scan/upload schedule capability, V2.3 signed read-only schedule-preview dispatch for separately opted-in clients, a classifier fix that keeps historical resolved failed upload counts visible without forcing `needs_attention` when no matching queue remains and current source evidence is otherwise healthy, reconciliation that preserves client-reported remote-action result code and summary aliases in dashboard action history, guarded V2.3 Schedule Apply for the Alynt uploader scan cadence only, clearer Schedule Apply history details showing sanitized cadence transitions, next run times, and pending client cadence-report states, clearer backup-source attention summaries on Sites and Site Detail, compact Sites-row source reason lines explaining why source evidence is fresh, policy-valid, optional, stale, missing, or queued, support-safe rollback readiness evidence for redacted Schedule Apply metadata, non-mutating Schedule Rollback Preview controls for separately opted-in clients with fresh rollback metadata, explicit Site Detail rollback-preview readiness states, compact rollback-preview readiness pills, support-safe Diagnostics aggregate counts for rollback-preview-supported and hidden sites, structure cleanup that splits Diagnostics local actions and Site Detail local record panels into focused traits, Site Detail guidance for locally revoked dashboard records, clearer Sites-row schedule capability hints, an archived-records view that keeps audit history available without crowding default monitoring workflows, archive-toggle helper layout polish for constrained admin screens, and reconciled roadmap status for completed dashboard slices. Schedule Apply requires a fresh successful matching preview, signed action dispatch, and the client's separate local Schedule Apply opt-in. V2.1 is limited to signed `scan_upload_now` intents that ask the client uploader to scan for ready packages and upload eligible items using its own local settings; V2.2 reconciles redacted client action results back into dashboard history; V2.3 can display redacted schedule capability evidence, request a redacted client-side schedule preview, apply the one previewed Alynt uploader scan cadence change, and request a non-mutating rollback preview when the client separately advertises support, but it cannot execute rollback, change schedules during rollback preview, disable schedules, change upload-worker cadence, change WPvivid or server-runner schedules, or otherwise act as generic schedule management. The dashboard does not create fresh backups, restore, delete, clean up, change settings, store Drime credentials, or run arbitrary commands.
...
Version 1 cannot run any remote actions. Version 0.1.28 includes one bounded Request Backup Now action after separate client-side V2.1 opt-in. That action asks the client uploader to scan for ready packages and upload eligible items; it cannot create fresh backups, restore, delete, clean up, change settings, expose Drime credentials, or run arbitrary commands. V2.2 only reconciles redacted client action evidence into dashboard history and diagnostics. V2.3 schedule visibility and schedule preview remain bounded, Schedule Apply is limited to one previewed future Alynt uploader scan cadence change after the client separately enables Schedule Apply, and Schedule Rollback Preview is non-mutating support evidence only; V2.3 cannot execute rollback, disable schedules, change upload-worker cadence, change WPvivid or server-runner schedules, create backups, restore, delete, clean up, or change credentials. The WPvivid external/optional policy is dashboard-local classification only and does not change the client site.
...
= 0.1.46 =
* Added compact rollback-preview readiness pills on Site Detail for hidden, waiting, blocked, expired, and ready states.
* Reconciled implementation-roadmap wording for completed dashboard slices.
* Preserved the rollback-preview boundary: no rollback execution, schedule mutation during preview, backup creation, restore, delete, cleanup, credential, Drime-token, or arbitrary-command actions were added.
...
= 0.1.44 =
* Added explicit Site Detail rollback-preview readiness states for hidden, waiting-for-apply-metadata, and ready non-mutating preview conditions.
* Added support-safe Diagnostics aggregate counts for rollback-preview support visibility and rollback-apply advertised canary evidence.
* Preserved the rollback-preview boundary: no rollback execution, schedule mutation during preview, backup creation, restore, delete, cleanup, credential, Drime-token, or arbitrary-command actions were added.

= 0.1.43 =
* Added non-mutating Schedule Rollback Preview dispatch and Site Detail controls for clients that separately advertise rollback-preview support and have fresh support-safe rollback metadata.
* Added rollback-preview action-history details and Diagnostics/action-summary counts.
* Preserved the V2.3 boundary: rollback preview does not execute rollback or change schedules.
...
= 0.1.41 =
* Added Site Detail Remote Action History filters for action type and dashboard state.
* Compacted long Remote Action History detail cells while keeping longer support-safe details available behind native disclosure.
* Split backup-source evidence helpers into focused traits during pre-release structure cleanup.

= 0.1.40 =
* Clarified V2.3 Schedule Apply wording so operators see that apply changes only the Alynt uploader scan cadence.
* Avoided misleading Schedule Preview and Schedule Apply history rows such as `Unknown -> every 15 minutes` while the dashboard is waiting for the client to report current or previous cadence evidence.
...
* Added an archived-records view and excluded archived records from default Sites, Attention, and scheduled polling contexts.
...
= 0.1.27 =
* Added dashboard-local scheduled polling Pause/Resume controls on Sites rows and Site Detail.
* Kept manual Check Now available while scheduled polling is paused, when polling credentials exist.
...
= 0.1.26 =
* Added clearer backup-source attention summaries on the Sites list and Site Detail screens so operators can see why a source needs review.
* Added support-safe rollback readiness evidence for redacted Schedule Apply metadata while keeping rollback unavailable.
* Preserved the current remote-action boundary: no backup creation, restore, delete, cleanup, credential, Drime-token, generic schedule-management, rollback, or arbitrary-command capabilities were added.
...
* Preserved support-safe Schedule Apply context fields in dashboard action history without storing raw payloads or secrets.

= 0.1.24 =
* Added guarded V2.3 Schedule Apply support for the Alynt scan/upload schedule. Apply can be dispatched only from a fresh successful matching preview and only when the client reports its separate local Schedule Apply opt-in.
* Extended new V2 action opt-in tokens to advertise `schedule_apply` while preserving the client-local mutation opt-in gate and excluding rollback, WPvivid/server-runner schedule mutation, backup creation, restore, delete, cleanup, settings, credential, Drime-token, arbitrary cron, and arbitrary-command actions.

= 0.1.23 =
* Preserved client-reported remote-action result codes and summaries when clients report `code` and `summary`, so schedule-preview action history can show the reconciled client result text.

= 0.1.22 =
* Added V2.3 signed, read-only schedule-preview dispatch for separately opted-in clients. The dashboard can request a redacted preview for the Alynt scan/upload schedule, but cannot apply, disable, roll back, or otherwise change schedules.
* Regenerated the translation template with release-artifact directories excluded.

= 0.1.21 =
* Kept historical resolved failed upload counts visible while preventing them from forcing `needs_attention` when no matching queue remains and current source evidence is otherwise healthy.
...
* Added a Site Detail toggle and at-a-glance source evidence labels for the policy while preserving attention for failed uploads and other hard failures.
...
= 0.1.16 =
* Added V2.2 dashboard-side reconciliation of sanitized client action evidence into matching dashboard action-history records.
* Added stale-action detection, clearer dashboard/client action-history columns, compact Sites-row latest-client-action hints, and support-safe Diagnostics action aggregates.
* Preserved the V1 read-only polling boundary and V2.1 scan/upload-now-only action boundary; no new remote powers were added.
