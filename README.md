...
- `schedule_apply` is limited to one target: the Alynt scan/upload cadence (`alynt_scan_upload`).
...
- Apply changes only future Alynt scan/upload cadence. It does not create a backup immediately.
- No rollback runtime behavior, WPvivid schedule changes, server-runner schedule changes, Drime credential storage, backup creation, restore, delete, cleanup, arbitrary cron, or arbitrary command actions are included.
...
Version 0.1.27 currently includes:
