...
- V2.3 schedule management remains a higher-risk gated phase because later slices may change persistent client backup behavior. The preview-only `alynt_scan_upload` schedule capability slice and non-mutating signed `schedule_preview` action are implemented, released, and deployed through dashboard `0.1.22` and uploader `0.5.18`. The next planned slice is a planning-only `schedule_apply` gate for `alynt_scan_upload` cadence changes only. No `schedule_apply` or `schedule_rollback` runtime behavior is coded yet.
