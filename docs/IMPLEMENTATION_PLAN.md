...
- keep `schedule_rollback_preview` dashboard dispatch/UI unavailable until a later dashboard implementation slice is separately approved;
...
Implementation status: implemented as a display-only row-hint slice. It does not add Sites-row controls, protocol changes, database writes, or remote actions.
